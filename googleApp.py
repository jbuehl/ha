...
class GoogleRequestHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            if debugData: syslog.syslog(pprint.pformat(data))
            ...
            if action == "get":
                ...
                try:
                    sensorName = data["queryResult"]["outputContexts"][0]["parameters"]["sensor.original"]
                except KeyError:
                    sensorName = sensor
                ...
                sensor = control
                try:
                    sensorName = data["queryResult"]["outputContexts"][0]["parameters"]["control.original"]
                except KeyError:
                    sensorName = sensor
                if parameters["state"] != "":
        ...
        try:
            ...
            self.sendResponse(answer+"The "+sensorName+(" are " if sensor[-1]=="s" else " is ")+convertState(state))
        except Exception as ex:
            syslog.syslog("exception: "+str(ex))
