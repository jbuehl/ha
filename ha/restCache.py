        ...
        while running:
            ...
            if serverName != self.selfRest:   # ignore the beacon from this service
                if serverName not in self.servers.keys():   # new server
                    debug('debugRestCache', self.name, timeStamp, "adding", serverName, serverAddr, serverTimeStamp, serverLabel)
                    ...
                    if serverTimeStamp > self.servers[serverName].timeStamp: # server resources changed
                        debug('debugRestCache', self.name, timeStamp, "updating", serverName, serverAddr, serverTimeStamp, serverLabel)
...
class RestCacheServer(HASensor):
    def __init__(self, name, addr, timeStamp, interface, event=None, group="", type="server", location=None, view=None, label="", interrupt=None):
