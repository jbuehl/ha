...
class Display(object):
    ...
    def start(self, block=False):
            ...
            for event in self.inputDevice.read_loop():
                ...
                if event.type == 1:         # BTN_TOUCH
                    if event.code == 330:
                        ...
                        if element:
                            if event.value == 0:    # up
                                element.releaseThread = LogThread(name=element.name+"releaseThread", target=element.release)
                                element.releaseThread.start()
                            elif event.value == 1:   # down
                                element.pressThread = LogThread(name=element.name+"pressThread", target=element.press)
        ...
        inputThread = LogThread(name="inputThread", target=InputThread)
        inputThread.start()
        ...
        updateThread = LogThread(name="updateThread", target=UpdateThread)
        updateThread.start()
